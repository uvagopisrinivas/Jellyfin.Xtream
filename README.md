...
- **v0.9.16** - Remove IDisableMediaSourceDisplay from VOD channel so Jellyfin probes remote streams for correct video/audio track discovery.
- **v0.9.15** - Fix video freeze and audio track mismatch: removed synthetic audio/video streams for VOD/Series so Jellyfin probes the actual container for correct stream indexes. Fixed SemaphoreFullException in VOD metadata provider (thread-safe semaphore with proper lifecycle).
